...
\author{Aakaash Nattanmai\\Cornell University \and Harshaan Chugh\\Cornell University}
\date{May 13, 2026}
...
push/pull traversal. We also add an optional replicated-CSR multi-GPU
prototype to test whether extra GPUs help without communication-aware frontier
exchange. On Perlmutter CPU nodes, we extend the project with 1D and 2D MPI
graph partitioning, bitmap frontier reductions, and SNAP road network
benchmarks. The fastest SpMV-style GPU variant wins on all six
...
ranks. The multi-GPU prototype shows negative scaling on 2 and 4 GPUs,
highlighting the cost of per-level frontier synchronization. These results
suggest that no single BFS formulation dominates: the best design depends on
whether memory bandwidth, frontier construction, communication volume, or graph
diameter is the limiting cost.
\end{abstract}
...
Our project asks how the choice of abstraction affects BFS performance across
parallel architectures. We started from the course suggestion to compare a
graph-first BFS with a linear-algebraic formulation inspired by GraphBLAS. We
then extended the study after the poster checkpoint with MPI implementations
that compare 1D and 2D partitioning and with an optional multi-GPU prototype.
Our hypothesis was that linear-algebraic GPU kernels would be competitive when
frontier operations could be expressed as regular sparse row scans, while
graph-first traversal would benefit when frontier sizes were small enough to
avoid scanning most vertices. For MPI, we expected 2D partitioning to reduce
communication at high rank counts, especially on graphs with enough edges to
amortize its extra setup. For multi-GPU BFS, we expected the result to depend
on whether the saved row-scan work exceeded the cost of synchronizing frontier
bitmaps between devices.

\section{Context and Contributions}

This is a course project for CS 5220 rather than a subproject in a larger
research effort. The code and benchmark scripts are reusable for future
parallel-computing coursework because the repository keeps the graph loader,
validation harness, CUDA kernels, MPI implementations, Slurm scripts, and raw
CSV outputs together. Within the group, Aakaash focused on GPU benchmarking,
multi-GPU evaluation, and integrating the GPU and multi-GPU results into the
final analysis. Harshaan focused on the MPI extension, SNAP road-network
benchmarking, and distributed-memory analysis. Both members contributed to
CUDA BFS implementation, validation, poster preparation, and final report
revision.
...
\begin{itemize}
    ...
    \item a replicated-CSR multi-GPU BFS prototype with 1, 2, and 4 GPU
    measurements;
...
\subsection{Multi-GPU Linear-Algebraic Prototype}

The optional multi-GPU extension keeps the linear-algebraic bitmap-pull
formulation but partitions the output rows across multiple GPUs on one
Perlmutter node. Each GPU stores a full CSR copy and owns a contiguous vertex
range for writing the next frontier. At every BFS level, the host broadcasts
the current frontier bitmap to all GPUs, launches the row-partitioned pull
kernel on each device, copies each partial next-frontier bitmap back to the
host, ORs those partial bitmaps together, and repeats. This replicated-CSR
prototype is simple and useful as a scaling experiment, but it is intentionally
not a production multi-GPU BFS: it pays a full host-mediated frontier exchange
every level and does not use peer-to-peer GPU communication or a distributed
CSR layout.
...
\subsection{Communication and Kernel Cost Model}
...
of 10 runs. Optional multi-GPU experiments ran on one 4-GPU Perlmutter node
and report the median of 3 runs for 1, 2, and 4 requested GPUs. Those timings
include the prototype's host-mediated bitmap synchronization between levels.
MPI experiments ran on Perlmutter CPU nodes with 10 trials for synthetic graphs
and 3 trials for SNAP road networks. MPI timings use \texttt{MPI\_Wtime} around
the BFS traversal.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/gpu_head_to_head.pdf}
...
\begin{table}[t]
\centering
\caption{Optional replicated-CSR multi-GPU BFS results. Speedup is relative to
the same prototype using one GPU, so values below 1.0 indicate slowdown.}
\label{tab:mgpu}
\small
\begin{tabular}{@{}l r r r@{}}
\toprule
Graph & 1 GPU ms & 2 GPU speedup & 4 GPU speedup \\
\midrule
large\_dense & 0.550 & 0.93x & 0.65x \\
large\_powerlaw & 0.599 & 0.90x & 0.72x \\
large\_sparse & 0.403 & 0.67x & 0.41x \\
roadNet-PA & 50.029 & 0.69x & 0.40x \\
roadNet-TX & 78.738 & 0.72x & 0.40x \\
roadNet-CA & 76.352 & 0.71x & 0.40x \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:mgpu} shows that the naive multi-GPU prototype does not scale:
2 GPUs are slower than 1 GPU on every input, and 4 GPUs are slower still. This
is an informative negative result. The per-level bitmap exchange is small in
absolute bytes, but BFS performs one exchange per level, and the prototype
routes every exchange through host memory. On road networks, this overhead is
especially visible because roadNet-PA, roadNet-TX, and roadNet-CA require 541,
722, and 554 BFS levels respectively. Extra GPUs reduce the number of output
rows scanned per device, but the saved row-scan work is smaller than the added
synchronization and device-management cost for these graphs.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/mpi_synthetic_speedup.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/mpi_snap_speedup.pdf}
...
The multi-GPU experiment reinforces the same point on a single node: simply
adding devices is not enough if every level performs a centralized frontier
merge.
...
savings of a distributed 2D BFS. The multi-GPU prototype similarly keeps a
replicated CSR on every GPU and synchronizes through the host, so it should be
read as a communication-cost probe rather than a tuned multi-GPU BFS. Finally,
GPU and MPI timings should not be interpreted as direct hardware comparisons;
...
$O(\sqrt{P})$ communication model. For multi-GPU BFS, the corresponding next
step is to avoid host-mediated full-frontier exchange by using peer-to-peer or
NCCL communication and a true distributed CSR layout. A follow-up experiment
should also include weak scaling on generated RMAT graphs and compare with a
tuned distributed GraphBLAS implementation.
...
networks. The optional multi-GPU prototype did not scale because per-level
frontier synchronization dominated the saved row-scan work. Together, these
experiments show why BFS remains a useful benchmark for parallel computing:
the best algorithm is not simply the one with the most parallelism, but the one
that best matches frontier structure to the memory and communication costs of
the target machine.
