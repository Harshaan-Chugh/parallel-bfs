...
\newcommand{\tabtodo}{\textcolor{todored}{TODO}}
...
\block{8.\ Results (head-to-head summary)}{
    ...
    load/print excluded).\ On every synthetic in block~\textbf{5}, the fastest
    SpMV kernel beats graph-first in our current timings; gaps are tight on
    sparse/dense random graphs (about $1.1\text{--}1.3\times$) and widen on
    power-law and chain stress cases.

    \vspace{0.2em}
    \textbf{Per-kernel SpMV sweep} (all variants in ms) is tabulated in
    \textbf{10} together with a \textbf{graph-first timing grid} placeholder---no
    per-variant README table yet for graph-first, so those cells are marked
    \tabtodo{} until we freeze a comparable breakout.
}
...
\block{10.\ Runtime (kernel ms)}{
    ...
    \raggedright
    \textbf{Linear-algebraic SpMV} (Perlmutter A100; matches
    \texttt{linear-algebraic/README.md}). Times in ms; \textbf{bold} = best per row.
    ...
    \begin{tabular}{@{}l rr rrrrr@{}}
        ...
        \texttt{m\_chain} & 2 & 999 & 20.19 & 21.57 & \textbf{19.02} & 32.11 & 23.95 \\
        \texttt{m\_dense} & 100 & 2 & 5.30 & 7.02 & 0.58 & 0.65 & \textbf{0.57} \\
        \texttt{m\_sparse} & 6 & 6 & 2.47 & \textbf{0.61} & 0.63 & 0.79 & 0.69 \\
        \midrule
        \multicolumn{8}{@{}l}{\itshape Large ($10^5$ vertices)} \\
        \texttt{l\_dense} & 100 & 4 & 4.76 & 14.00 & 4.44 & 4.67 & \textbf{4.41} \\
        \texttt{l\_power} & 40 & 3 & 2.81 & 2.56 & 2.46 & 4.38 & \textbf{2.41} \\
        \texttt{l\_sparse} & 10 & 7 & 11.34 & 1.79 & 18.15 & 1.79 & \textbf{1.58} \\
        \bottomrule
    \end{tabular}

    \vspace{0.55em}
    \raggedright
    \textbf{Graph-first} (direction-optimizing); per-kernel table \tabtodo---same
    graph names, to be filled from the graph-first driver.

    \vspace{0.2em}
    \centering
    \begin{tabular}{@{}l rr rrr@{}}
        \toprule
        & $\bar d$ & dep & top-down & bottom-up & hybrid \\
        \midrule
        \texttt{m\_chain} & 2 & 999 & \tabtodo & \tabtodo & \tabtodo \\
        \texttt{m\_dense} & 100 & 2 & \tabtodo & \tabtodo & \tabtodo \\
        \texttt{m\_sparse} & 6 & 6 & \tabtodo & \tabtodo & \tabtodo \\
        \midrule
        \texttt{l\_dense} & 100 & 4 & \tabtodo & \tabtodo & \tabtodo \\
        \texttt{l\_power} & 40 & 3 & \tabtodo & \tabtodo & \tabtodo \\
        \texttt{l\_sparse} & 10 & 7 & \tabtodo & \tabtodo & \tabtodo \\
        \bottomrule
    \end{tabular}
}
